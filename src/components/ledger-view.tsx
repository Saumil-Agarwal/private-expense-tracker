...
export function LedgerView({ summary = false }: { summary?: boolean }) {
  ...
  if (summary) return <div className="summary-content">
    ...
    <section className="workings-card"><h2>Workings</h2>{transactions.map((transaction) => { const working = totals.workings.find((item) => item.id === transaction.id)!; return <article key={transaction.id}><div><strong>{transaction.merchant}</strong><span>{transaction.occurred_on}{transaction.groups?.name ? ` · ${transaction.groups.name}` : ""}</span></div><strong>{formatInr(transaction.amount_paise)}</strong><p>{transaction.allocations.length ? transaction.allocations.map((allocation) => `${allocation.person} ${formatInr(allocation.amount_paise)}`).join(" + ") : "Unresolved — no shares assigned"}</p>{transaction.status === "confirmed" && <small>{working.balanced ? "Verified" : `Mismatch by ${formatInr(Math.abs(working.differencePaise))}`}</small>}</article>; })}</section>
  </div>;
