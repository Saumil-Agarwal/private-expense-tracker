...
export function LedgerView({ summary = false, trash = false }: { summary?: boolean; trash?: boolean }) {
  ...
  if (summary) return <div className="summary-content">
    ...
    <section className="insight-card"><p className="eyebrow">Share of each</p>{totals.participantTotals.length ? <ul className="allocation-list participant-summary-list">{totals.participantTotals.map((item) => <li key={item.person}><Link href={`/people/${encodeURIComponent(item.person)}`}><span>{item.person}</span><strong>{formatInr(item.amountPaise)}</strong></Link></li>)}</ul> : <p>No confirmed shares in these dates.</p>}</section>
