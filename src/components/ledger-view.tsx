...
export function LedgerView({ summary = false }: { summary?: boolean }) {
  ...
  const [transactions, setTransactions] = useState<Transaction[]>([]); const [error, setError] = useState(""); const [locked, setLocked] = useState(false); const [loading, setLoading] = useState(true);
  const load = useCallback(async () => {
    setLoading(true); setError("");
    try {
      const params = summary ? `?from=${from}&to=${to}` : "";
      const response = await fetch(`/api/expenses${params}`, { headers: { "x-telegram-init-data": window.Telegram?.WebApp?.initData ?? "" } });
      ...
      setLocked(false); setTransactions((await response.json()).transactions);
  ...
  if (locked) return <UnlockForm onUnlocked={() => void load()} />;
  ...
  if (summary) return <div className="summary-content">
    ...
    <section className="workings-card"><h2>Workings</h2>{transactions.map((transaction) => <article key={transaction.id}><div><strong>{transaction.merchant}</strong><span>{transaction.occurred_on}{transaction.groups?.name ? ` · ${transaction.groups.name}` : ""}</span></div><strong>{formatInr(transaction.amount_paise)}</strong><p>{transaction.allocations.length ? transaction.allocations.map((allocation) => `${allocation.person} ${formatInr(allocation.amount_paise)}`).join(" + ") : "Unresolved — no shares assigned"}</p></article>)}</section>
  </div>;
