...
import { ExpenseEditor, type EditableExpense } from "./expense-editor";
...
export function ExpenseDetail({ id }: { id: string }) {
  ...
  if (editing) return <ExpenseEditor expense={expense} onCancel={() => setEditing(false)} />;
  ...
  return <div className="expense-detail">
    ...
    <div className="detail-actions">{expense.deleted_at ? <><button className="button-primary" disabled={acting} onClick={() => void update("restore")}>Restore expense</button><button className="button-danger" disabled={acting} onClick={() => void permanentlyDelete()}>Delete permanently</button></> : <><button className="button-primary" disabled={acting} onClick={() => setEditing(true)}>Edit expense</button><button className="button-quiet" disabled={acting} onClick={() => void update("trash")}>Move to Trash</button></>}</div>
  </div>;
}
