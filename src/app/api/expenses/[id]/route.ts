...
const detailSelection = "id,occurred_on,merchant,amount_paise,currency,status,notes,source,deleted_at,categories(name),groups(id,name),allocations(amount_paise,people(name)),transaction_items(id,name,quantity,amount_paise,people(name))";
...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    const row = data as unknown as {
      allocations?: Array<{ amount_paise: number; people?: { name: string } | null }>;
      transaction_items?: Array<{ id: string; name: string; quantity: number; amount_paise: number; people?: { name: string } | null }>;
      [key: string]: unknown;
    };
    return NextResponse.json({ expense: {
      ...row,
      allocations: (row.allocations ?? []).map((allocation) => ({ person: allocation.people?.name ?? "Unknown", amount_paise: allocation.amount_paise })),
      items: (row.transaction_items ?? []).map((item) => ({ id: item.id, name: item.name, quantity: item.quantity, amount_paise: item.amount_paise, owner: item.people?.name ?? null })),
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    if (body?.action !== "trash" && body?.action !== "restore") return NextResponse.json({ error: "Invalid expense action" }, { status: 400 });
    const [{ id }, { userId, supabase }] = await Promise.all([context.params, getLocalOwnerContext()]);
